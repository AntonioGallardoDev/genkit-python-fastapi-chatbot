...
- Autenticación por usuarios (Argon2id + JWT)
- Preparado para evolución a chatbot corporativo con usuarios/roles y RAG
---
...
- `data/` → archivos locales (NO versionar)
- `data/memory/` → sesiones JSON (memoria por sesión)
- `data/auth/` → usuarios/roles (MVP corporativo, en fichero)
- `data/audit/` → logs de auditoría (eventos de auth, admin, etc.)
...
JWT + API Key (doble capa temporal)
...
### Estado de la siguiente fase (corporativo)

✅ Definición de RBAC en fichero:
- `data/auth/roles.json`
- `data/auth/users.json` (con `password_hash`, sin passwords en claro)

⏭️ Próximo paso:
- Repositorio de usuarios en fichero (load/save/locks/escritura atómica) + tests
- Login JWT + endpoint `/me` (manteniendo API key en transición)
